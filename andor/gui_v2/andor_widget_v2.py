...
from .live_view_widget_v2 import AndorLiveViewWidgetV2
from .workers import LiveAcqThread, SingleAcqThread


class AndorWidgetV2(QtWidgets.QWidget):
    ...
    def _build_live_view(self, parent_layout):
        self.live_view = AndorLiveViewWidgetV2(
            cam=None,
            title="Live View",
            default_linecut_width=DEFAULT_LINECUT_WIDTH,
        )
        parent_layout.addWidget(self.live_view, 1)
    ...
    def on_connect(self):
        ...
        self.live_view.set_camera(self.cam)
    ...
    def on_disconnect(self):
        self.on_stop()
        self.live_view.set_camera(None)
        try:
            self.cam.disconnect()
        except Exception:
            pass
    ...
    def on_spec_connect(self):
        ...
        if ok:
            ...
            self.live_view.refresh_wavelength_axis(force=True)
        else:
    ...
    def on_spec_disconnect(self):
        ...
        self.specDisconnectBtn.setEnabled(False)
        self.live_view.set_wavelength_axis_enabled(False)

    def on_refresh_axis(self):
        try:
            wl = self.live_view.refresh_wavelength_axis(force=True)
            if wl is not None:
                self.statusBar().showMessage("Axis refreshed")
            else:
                self.statusBar().showMessage("Axis refresh failed: no data")
        except Exception as exc:
    ...
    def on_new_frame(self, fr: dict):
        ...
        self._last_frame = np.fliplr(np.flipud(raw))
        self.live_view.update_frame(fr)
