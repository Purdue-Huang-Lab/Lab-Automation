...
class DevicePanel(QtWidgets.QGroupBox):
    ...
    def poll_once(self, oc_limit_a: Optional[float], oc_trip_samples: int, allow_poll: bool) -> Tuple[Optional[float], Optional[float], bool]:
        ...
        try:
            vi = self.dev.read_vi_with_timeout(POLL_TIMEOUT_MS)   # ONE read per poll tick
            v = float(vi.v)
            iA = float(vi.i)
    ...
    # ---- Stop (abort whichever is running) ----
    ...
    def on_stop_any(self):
        ...
        # disable step/stop buttons immediately (threads will unwind)
