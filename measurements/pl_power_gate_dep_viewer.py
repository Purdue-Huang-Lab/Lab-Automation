...
_POWER_RE = re.compile(r"(?:^|_)P(?P<val>[+-]?(?:m)?[0-9]+(?:p[0-9]+)?(?:em?[0-9]+)?)\s*(?P<unit>nW|uW|mW|W)(?=_|\.|$)")
...
def _parse_token(token: str) -> Optional[float]:
    ...
    if token.startswith("m"):
        sign *= -1.0
        token = token[1:]
    # em<N> in filename encodes e-<N> (e.g. em16 → e-16)
    token = re.sub(r'em(\d+)', r'e-\1', token)
    try:
        val = sign * float(token.replace("p", "."))
    except Exception:
        return None
    # Values smaller than 1e-9 are effectively zero (encoding artefact)
    if abs(val) < 1e-9:
        return 0.0
    return val
...
def sg_smooth_2d(
    ...
    """Savitzky-Golay smoothing on a 2-D map (shape: wavelength × sweep).

    win_sweep: window along axis-1 (sweep/gate/power).  0 or <3 = disabled.
    win_wl:    window along axis-0 (wavelength/energy). 0 or <3 = disabled.
    Windows are forced to odd values.
    """
    try:
    ...
    def _smooth_rows(arr: np.ndarray, win: int, order: int) -> np.ndarray:
        """Smooth along axis=1 (each row independently)."""
        ...
        for i in range(arr.shape[0]):
            ...
            if np.count_nonzero(mask) < win:
                continue
            if mask.all():
                res[i, :] = savgol_filter(row, win, order, mode="mirror")
            else:
                xi = np.arange(row.size)
                filled = np.interp(xi, xi[mask], row[mask])
                res[i, :] = savgol_filter(filled, win, order, mode="mirror")
                res[i, ~mask] = np.nan
        return res

    def _smooth_cols(arr: np.ndarray, win: int, order: int) -> np.ndarray:
        """Smooth along axis=0 (each column independently)."""
        ...
        for j in range(arr.shape[1]):
            ...
            if np.count_nonzero(mask) < win:
                continue
            if mask.all():
                res[:, j] = savgol_filter(col, win, order, mode="mirror")
            else:
                xi = np.arange(col.size)
                filled = np.interp(xi, xi[mask], col[mask])
                res[:, j] = savgol_filter(filled, win, order, mode="mirror")
                res[~mask, j] = np.nan
        return res

    out = _smooth_rows(out, win_sweep, ord_sweep)
    out = _smooth_cols(out, win_wl, ord_wl)
    return out
...
class OfflineLiveView(AndorLiveViewWidget):
    ...
    def update_frame(self, fr: dict) -> None:
        """Offline data is already horizontally corrected — apply vertical flip only."""
        img = fr.get("image")
        img8 = fr.get("image8")
        if img is None:
            return
    ...
    def _update_cursor_overlays(self, h: int, w: int) -> None:
        """Cursor overlay for offline data — no fliplr, so display col == raw col."""
        ...
        y_raw = h - 1 - y  # flipud: display row y == raw row h-1-y
        if not (0 <= y_raw < h):
            self._crosshair_h.set_visible(False)
            self._crosshair_v.set_visible(False)
            return

        x_raw = x  # no fliplr — display col == raw col
        try:
        ...
        width_lc = int(self.linecutWidthSpin.value())
        hcut = self._linecut_horizontal(self._last_frame_raw, y_raw, width=width_lc)
        vcut = self._linecut_vertical(self._last_frame_raw, x_raw, width=width_lc)

        if hcut is not None:
            x_axis = self._get_x_axis_data(w)
            self.line_h.set_data(x_axis, hcut)  # no reversal — offline data has no fliplr
            self.ax_h.relim()
...
class PowerGateDepWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, frames: List[PLFrame], defaults: ViewerDefaults, data_dir: Path):
        super().__init__()
        self.frames = frames
        ...
        self._bg_single: Optional["PLFrame"] = None   # single-file BG
        self._bg_grid: Dict[Tuple, "PLFrame"] = {}     # folder BG keyed by (power_key, gate_key)
    ...
    def _apply_sg_smooth(self, m: np.ndarray) -> np.ndarray:
        win_sweep = int(self.sgSweepWinSpin.value()) if self.sgSweepCheck.isChecked() else 0
        ord_sweep = int(self.sgSweepOrdSpin.value()) if self.sgSweepCheck.isChecked() else 1
        win_wl = int(self.sgWlWinSpin.value()) if self.sgWlCheck.isChecked() else 0
        ord_wl = int(self.sgWlOrdSpin.value()) if self.sgWlCheck.isChecked() else 1
        if win_sweep < 3 and win_wl < 3:
            return m
        return sg_smooth_2d(m, win_sweep, ord_sweep, win_wl, ord_wl)

    def _browse_folder(self) -> None:
        chosen = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select data folder", str(self.data_dir)
        )
        if not chosen:
            return
        new_dir = Path(chosen)
        frames, defaults = load_frames(new_dir)
        if not frames:
        ...
        self._build_grid()
        self._path_lbl.setText(str(self.data_dir))
    ...
    def _build_ui(self) -> None:
        ...
        self._path_lbl = QtWidgets.QLabel(str(self.data_dir))
        self._path_lbl.setWordWrap(True)
        grid.addWidget(self._path_lbl, 0, 1, 1, 6)
        ...
        grid.addWidget(QtWidgets.QLabel("SG smooth:"), 6, 0)
        self.sgSweepCheck = QtWidgets.QCheckBox("Sweep")
        self.sgSweepWinSpin = QtWidgets.QSpinBox()
        self.sgSweepWinSpin.setRange(3, 501)
        self.sgSweepWinSpin.setSingleStep(2)
        self.sgSweepWinSpin.setValue(5)
        self.sgSweepWinSpin.setFixedWidth(65)
        self.sgSweepWinSpin.setToolTip("SG window along sweep axis (odd integer)")
        self.sgSweepOrdSpin = QtWidgets.QSpinBox()
        self.sgSweepOrdSpin.setRange(1, 10)
        self.sgSweepOrdSpin.setValue(2)
        self.sgSweepOrdSpin.setFixedWidth(50)
        self.sgSweepOrdSpin.setToolTip("SG polynomial order along sweep axis")
        grid.addWidget(self.sgSweepCheck, 6, 1)
        grid.addWidget(self.sgSweepWinSpin, 6, 2)
        grid.addWidget(self.sgSweepOrdSpin, 6, 3)

        self.sgWlCheck = QtWidgets.QCheckBox("Along \u03bb")
        self.sgWlWinSpin = QtWidgets.QSpinBox()
        self.sgWlWinSpin.setRange(3, 501)
        self.sgWlWinSpin.setSingleStep(2)
        self.sgWlWinSpin.setValue(5)
        self.sgWlWinSpin.setFixedWidth(65)
        self.sgWlWinSpin.setToolTip("SG window along wavelength axis (odd integer)")
        self.sgWlOrdSpin = QtWidgets.QSpinBox()
        self.sgWlOrdSpin.setRange(1, 10)
        self.sgWlOrdSpin.setValue(2)
        self.sgWlOrdSpin.setFixedWidth(50)
        self.sgWlOrdSpin.setToolTip("SG polynomial order along wavelength axis")
        grid.addWidget(self.sgWlCheck, 6, 4)
        grid.addWidget(self.sgWlWinSpin, 6, 5)
        grid.addWidget(self.sgWlOrdSpin, 6, 6)

        # Row 7 — background subtraction + reverse wavelength axis
        self.bgFileBtn = QtWidgets.QPushButton("BG File…")
        self.bgFileBtn.setFixedWidth(90)
        self.bgFolderBtn = QtWidgets.QPushButton("BG Folder…")
        self.bgFolderBtn.setFixedWidth(100)
        self.bgLbl = QtWidgets.QLabel("No background")
        self.bgLbl.setStyleSheet("font-style: italic; color: #888;")
        self.bgSubtractCheck = QtWidgets.QCheckBox("Subtract BG")
        self.bgSubtractCheck.setEnabled(False)
        self.reverseWlCheck = QtWidgets.QCheckBox("Reverse λ axis")

        grid.addWidget(QtWidgets.QLabel("Background:"), 7, 0)
        grid.addWidget(self.bgFileBtn, 7, 1)
        grid.addWidget(self.bgFolderBtn, 7, 2)
        grid.addWidget(self.bgLbl, 7, 3, 1, 3)
        grid.addWidget(self.bgSubtractCheck, 7, 6)
        grid.addWidget(self.reverseWlCheck, 7, 7)
        ...
        info_box = QtWidgets.QGroupBox("Additional Info")
        info_layout = QtWidgets.QVBoxLayout(info_box)
        info_layout.setContentsMargins(6, 6, 6, 6)
        info_layout.setSpacing(6)
        self.infoFig = Figure(figsize=(8, 2.2), dpi=100)
        self.infoCanvas = FigureCanvas(self.infoFig)
        self.infoToolbar = NavigationToolbar(self.infoCanvas, info_box)
        self.axInfo = self.infoFig.add_subplot(111)
        self.axInfoTop = None
        info_layout.addWidget(self.infoToolbar)
        info_layout.addWidget(self.infoCanvas)
        ...
        self.sgSweepCheck.toggled.connect(lambda _: self._refresh_all())
        self.sgSweepWinSpin.valueChanged.connect(lambda _: self._refresh_all())
        self.sgSweepOrdSpin.valueChanged.connect(lambda _: self._refresh_all())
        self.sgWlCheck.toggled.connect(lambda _: self._refresh_all())
        self.sgWlWinSpin.valueChanged.connect(lambda _: self._refresh_all())
        self.sgWlOrdSpin.valueChanged.connect(lambda _: self._refresh_all())
        self.bgFileBtn.clicked.connect(self._on_load_bg_file)
        self.bgFolderBtn.clicked.connect(self._on_load_bg_folder)
        self.bgSubtractCheck.toggled.connect(lambda _: self._refresh_all())
        self.reverseWlCheck.toggled.connect(self._on_reverse_wl_changed)
    ...
    def _cropped_frame_data(self, fr: PLFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.reverseWlCheck.isChecked():
            img = img[:, ::-1]
            # wl_c is kept unchanged: we're correcting the pixel order, not the wavelength labels
    ...
    # Background helpers
    # -----------------
    def _on_load_bg_file(self) -> None:
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Select background file", str(self.data_dir), "Andor ASCII (*.asc);;All files (*)"
        )
        if not path:
            return
        try:
            meta, wl, image = load_andor_ascii(Path(path))
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "Error loading BG", str(e))
            return
        p_n, vf_n, vb_n = parse_from_name(Path(path).name)
        self._bg_single = PLFrame(
            path=Path(path), power_w=p_n, v_front=vf_n, v_back=vb_n,
            wavelength_nm=wl, image=image, meta=meta, mtime=float(Path(path).stat().st_mtime),
        )
        self._bg_grid = {}
        self.bgLbl.setText(f"File: {Path(path).name}")
        self.bgSubtractCheck.setEnabled(True)
        self.bgSubtractCheck.setChecked(True)
        self._refresh_all()

    def _on_load_bg_folder(self) -> None:
        chosen = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select background folder", str(self.data_dir)
        )
        if not chosen:
            return
        bg_frames, _ = load_frames(Path(chosen))
        if not bg_frames:
            QtWidgets.QMessageBox.warning(self, "No data", f"No .asc files found in:\n{chosen}")
            return
        self._bg_single = None
        self._bg_grid = {}
        default_p = normalize_key(bg_frames[0].power_w) if bg_frames else 0.0
        default_vf = normalize_key(bg_frames[0].v_front) if bg_frames else 0.0
        if default_p is None:
            default_p = 0.0
        if default_vf is None:
            default_vf = 0.0
        for fr in bg_frames:
            p = normalize_key(fr.power_w)
            vf = normalize_key(fr.v_front)
            p = default_p if p is None else p
            vf = default_vf if vf is None else vf
            key = (float(p), float(vf))
            old = self._bg_grid.get(key)
            if old is None or fr.mtime > old.mtime:
                self._bg_grid[key] = fr
        self.bgLbl.setText(f"Folder: {Path(chosen).name} ({len(self._bg_grid)} files)")
        self.bgSubtractCheck.setEnabled(True)
        self.bgSubtractCheck.setChecked(True)
        self._refresh_all()

    def _get_bg_for_frame(self, fr: PLFrame) -> Optional[PLFrame]:
        """Return the background PLFrame that best matches fr, or None."""
        if self._bg_single is not None:
            return self._bg_single
        if not self._bg_grid:
            return None
        p = float(normalize_key(fr.power_w) or self._default_power_key)
        vf = float(normalize_key(fr.v_front) or self._default_gate_key)
        key = (p, vf)
        bg = self._bg_grid.get(key)
        if bg is not None:
            return bg
        # Fallback: same gate, any power
        same_gate = [(k, v) for k, v in self._bg_grid.items() if k[1] == vf]
        if same_gate:
            same_gate.sort(key=lambda kv: abs(kv[0][0] - p))
            return same_gate[0][1]
        # Fallback: same power, any gate
        same_power = [(k, v) for k, v in self._bg_grid.items() if k[0] == p]
        if same_power:
            same_power.sort(key=lambda kv: abs(kv[0][1] - vf))
            return same_power[0][1]
        return None

    def _get_bg_cropped(self, fr: PLFrame) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """Return (bg_img_cropped, bg_wl_cropped) for the frame, or None."""
        bg_fr = self._get_bg_for_frame(fr)
        if bg_fr is None:
            return None
        bg_img = apply_crop(bg_fr.image, self._crop)
        bg_wl = bg_fr.wavelength_nm
        if bg_wl is None:
            bg_wl = np.arange(bg_fr.image.shape[1], dtype=float)
        bg_wl_c = crop_axis(bg_wl, self._crop, bg_fr.image.shape[1])
        if self.reverseWlCheck.isChecked():
            bg_img = bg_img[:, ::-1]
            # bg_wl_c unchanged: pixel order corrected, wavelength labels stay
        return bg_img, bg_wl_c

    def _on_reverse_wl_changed(self) -> None:
        # Reversal is baked into the cached cropped images — must clear cache
        self._cropped_image_cache.clear()
        self._cropped_wl_cache.clear()
        self._refresh_all()
    ...
    def _update_shared_image(self) -> None:
        ...
        display_image = np.asarray(fr.image, dtype=float)
        if self.reverseWlCheck.isChecked():
            display_image = display_image[:, ::-1]
        if self.bgSubtractCheck.isChecked():
            bg_fr = self._get_bg_for_frame(fr)
            if bg_fr is not None:
                bg_img = np.asarray(bg_fr.image, dtype=float)
                if self.reverseWlCheck.isChecked():
                    bg_img = bg_img[:, ::-1]
                if bg_img.shape == display_image.shape:
                    display_image = display_image - bg_img
        wl_display = fr.wavelength_nm  # wavelength labels stay fixed; only pixels are flipped
        self.liveView.set_wavelength_axis(wl_display)
        self.liveView.update_frame({"image": display_image, "image8": None})
    ...
    def _build_map_for_power_mode(self, row_raw: int, width: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        for p in self._powers:
            ...
            if lc is None:
                ...
                continue
            if self.bgSubtractCheck.isChecked():
                bg_data = self._get_bg_cropped(fr)
                if bg_data is not None:
                    bg_img, bg_wl = bg_data
                    bg_lc = linecut_horizontal(bg_img, row_raw, width)
                    if bg_lc is not None:
                        bg_lc = np.asarray(resample_linecut(bg_wl, bg_lc, wl), dtype=float)
                        lc = np.asarray(lc, dtype=float) - bg_lc
    ...
    @staticmethod
    def _integrate_over_energy(wl_nm: np.ndarray, map_data: np.ndarray) -> np.ndarray:
        wl = np.asarray(wl_nm, dtype=float).ravel()
        e = EV_NM / wl
        out = np.full((map_data.shape[1],), np.nan, dtype=float)
        for j in range(map_data.shape[1]):
            col = np.asarray(map_data[:, j], dtype=float)
            mask = np.isfinite(col) & np.isfinite(e)
            ...
            ex = ex[order]
            iy = iy[order]
            out[j] = float(np.trapz(iy, ex))
        return out

    def _update_info_plot(self, x: np.ndarray, y_wl: np.ndarray, map_data: np.ndarray) -> None:
        if self.axInfoTop is not None:
            try:
                self.axInfoTop.remove()
            except Exception:
                pass
            self.axInfoTop = None
        self.axInfo.clear()
        if self._current_mode != self.MODE_GATE:
            self.axInfo.set_title("Gate-integrated intensity (available in Gate mode)")
            self.axInfo.set_xlabel("Front gate (V)")
            self.axInfo.set_ylabel("Integrated I (a.u.)")
            self.infoCanvas.draw_idle()
            return

        integ = self._integrate_over_energy(y_wl, map_data)
        self.axInfo.plot(x, integ, marker="o", ms=3, lw=1.2, color="#2e7d32")
        self.axInfo.set_title("Integrated linecut intensity vs front gate")
        self.axInfo.set_xlabel("Front gate (V)")
        self.axInfo.set_ylabel("Integrated I over energy")
        self.axInfo.grid(True, alpha=0.3)
        valid = np.isfinite(x) & np.isfinite(integ)
        if np.count_nonzero(valid) > 0:
            xv = x[valid]
            yv = integ[valid]
            idx_cn = int(np.argmin(yv))
            v_cn = float(xv[idx_cn])
            y_cn = float(yv[idx_cn])
            self.axInfo.axvline(v_cn, color="#c62828", lw=1.0, ls="--", alpha=0.9)
            self.axInfo.plot([v_cn], [y_cn], marker="o", ms=5, color="#c62828")
            self.axInfo.text(
                0.02,
                0.95,
                f"Estimated CN: Vf={v_cn:.4g} V",
                transform=self.axInfo.transAxes,
                ha="left",
                va="top",
                fontsize=9,
                color="#c62828",
        ...
        self.axInfoTop = self.axInfo.twiny()
        self.axInfoTop.set_xlim(self.axInfo.get_xlim())
        self.axInfoTop.set_xlabel("Back gate (V)")
        nticks = min(8, x.size)
        idx = np.linspace(0, x.size - 1, nticks).astype(int)
        xt = x[idx]
        xb = self._backs_for_gate[idx] if self._backs_for_gate.size == x.size else np.full_like(xt, np.nan, dtype=float)
        labels = [f"{float(v):.3g}" if np.isfinite(v) else "" for v in xb]
        self.axInfoTop.set_xticks(xt)
        self.axInfoTop.set_xticklabels(labels)
        self.infoCanvas.draw_idle()
    ...
    def _update_maps(self) -> None:
        ...
        if self._current_mode == self.MODE_GATE:
            x, y_nm, m_nm, _ = self._build_map_for_gate_mode(row_raw, width)
            ...
            x, y_nm, m_nm, _ = self._build_map_for_power_mode(row_raw, width)
        ...
        # Apply SG smoothing; compute derivative from smoothed map
        m_smooth = self._apply_sg_smooth(m_nm)
        if self._current_mode == self.MODE_GATE:
            d_smooth = derivative_vs_x_nan(m_smooth, x)
        else:
            d_smooth = derivative_dlogp_nan(m_smooth, self._powers)

        y_energy, m_display = spectral_map_nm_to_energy(y_nm, m_smooth)
        _, d_display = spectral_map_nm_to_energy(y_nm, d_smooth)
        y_label = "Energy (eV)"
        max_abs = float(self.derivMaxSpin.value())
        self.derivView.set_manual_symmetric_max(max_abs if max_abs > 0 else None)
        self.mapView.set_map(x, y_energy, m_display, x_label=x_label, y_label=y_label, symmetric=False)
        self.derivView.set_map(x, y_energy, d_display, x_label=x_label, y_label=y_label, symmetric=True)
        ...
        self._update_info_plot(x, y_nm, m_nm)
...
def main() -> None:
    try:
        from measurements.config import DATA_DIR as _DATA_DIR
    ...
    if not any(data_dir.glob("*.asc")):
        chosen = QtWidgets.QFileDialog.getExistingDirectory(
            None, "Select data folder", str(data_dir)
        )
        if chosen:
    ...
    if not frames:
        print(f"No .asc files found in {data_dir}")
        return

    win = PowerGateDepWindow(frames, defaults, data_dir)
    win.show()
    sys.exit(app.exec_())
